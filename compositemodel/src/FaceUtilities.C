...
//===========================================================================
  void
  FaceUtilities::getBoundaryData2(ftSurface* face, int nmb_sample, 
				  vector<SamplePointData2>& sample_points)
//===========================================================================
  {
    double angtol = 0.01;
    double curvtol = 0.01;
	...
	Point kvec1, kvec2;
	double k1, k2;
	CurvatureAnalysis::principalCurvatures(*surf, par[0], par[1],
					       k1, kvec1, k2, kvec2);
	...
	for (kr=0; kr<faces_par.size(); ++kr)
	  {
	    ...
	    shared_ptr<ParamSurface> surf2 = faces_par[kr].first->surface();
	    ...
	    Point kvec3, kvec4;
	    double k3, k4;
	    CurvatureAnalysis::principalCurvatures(*surf2, par[0], par[1],
						   k3, kvec3, k4, kvec4);
	    ...
	    ang = std::max(kvec1.angle(kvec3), kvec2.angle(kvec4));
	    if (fabs(k1-k3) > curvtol || fabs(k2-k4) > curvtol || ang > angtol)
	      curv_OK = false;
	  }

	int type = 3 - (normal_OK) - (curv_OK);
	sample_points.push_back(SamplePointData2(pos, normal, kvec1, k1, kvec2,
						 k2, type, face, par[0], par[1],
						 edges[ki].get(), t1));
  ...
  //===========================================================================
  void
  FaceUtilities::getInnerData2(ftSurface* face, int nmb_sample_u, 
			      int nmb_sample_v, 
			      vector<SamplePointData2>& sample_points)
//===========================================================================
  {
	  ...
	  for (ki=0, upar=domain.umin()+udel; ki<nmb_sample_u-2; ++ki, upar+=udel)
	    {
	      ...
	      Point kvec1, kvec2;
	      double k1, k2;
	      CurvatureAnalysis::principalCurvatures(*surf, upar, vpar,
						     k1, kvec1, k2, kvec2);

	      sample_points.push_back(SamplePointData2(der[0], normal, kvec1, k1,
						       kvec2, k2, 0, face, upar, vpar));
	    }
      }
    else
      {
